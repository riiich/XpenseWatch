...
export function AppProvider({ children }: { children: ReactNode }) {
	...
	const getAccounts = async () => {
		...
		const transactionRes = await fetch("http://localhost:5095/api/accounts", {
			method: "GET",
			headers: {
	...
	const getCategories = async () => {
		const categoryRes = await fetch("http://localhost:5095/api/categories", {
			method: "GET",
			headers: {
				Authorization: `Bearer ${localStorage.getItem("token")}`,
			},
	...
	const getTransactions = async () => {
		...
		const res = await fetch(`http://localhost:5095/api/transactions/${selectedAccId}/transactions`, {
			method: "GET",
			headers: {
				Authorization: `Bearer ${localStorage.getItem("token")}`,
			},
	...
	const deleteTransaction = async (accId: number, txId: number) => {
		// api call to delete
		const res = await fetch(`http://localhost:5095/api/transactions/${txId}`, {
			method: "DELETE",
            headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
