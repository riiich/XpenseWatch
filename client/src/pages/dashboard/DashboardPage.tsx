		...
		<>
			{isLoading ? (
				...
				<div className="flex h-screen bg-[#060c14] overflow-hidden">
					...
					<main className="flex-1 overflow-y-auto px-10 py-8">
								...
								{isTransactionLoading ? (
									...
									<>
										...
										<div className="flex items-center gap-3 px-3 pb-2 border-b border-[#1e293b] text-slate-100 mb-1">
											<span className="text-[11px] tracking-[0.12em] w-14">DATE</span>
											<span className="w-20" />
						...
						{activeTab === "ai" && (
							<AIAdvisorTab
								...
								setInsight={(value) =>
									setInsights((prev) => ({ ...prev, [account.id]: value }))
								}
								open={insightOpen[account.id] ?? false}
								setOpen={(value) =>
									setInsightOpen((prev) => ({ ...prev, [account.id]: value }))
								}
							/>
						)}
					</main>
					...
					{/* Modals */}
					{showAddTx && (
						<AddTransactionForm onClose={() => setShowAddTx(false)} categories={categories} />
					)}
					{showAddAcc && <AddAccountForm onClose={() => setShowAddAcc(false)} />}
				</div>
			)}
		</>
	);
