...
export default function DashboardPage() {
  const { addAccount, addTransaction, deleteTransaction, categories, addCategory, selectedAccId } =
    useApp();
  ...
  return (
    <div className="flex h-screen bg-[#060c14] overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto px-10 py-8">
            ...
            <div className="text-right">
              ...
              <span
                className={`font-display text-[26px] font-semibold ${
                  account.balance >= 0 ? "text-slate-100" : "text-rose-400"
                }`}
              >
          ...
          <div className="animate-fade-in">
            {account.transactions.length === 0 ? (
              <div className="text-center py-20">
                <div className="text-6xl text-[#1e293b] mb-4">◇</div>
                <p className="text-slate-600 text-sm mb-5">No transactions yet</p>
                <button
                  onClick={() => setShowAddTx(true)}
              ...
              <>
                ...
                <div className="flex items-center gap-3 px-3 pb-2 border-b border-[#1e293b] mb-1">
                  <span className="text-[9px] tracking-[0.12em] text-slate-600 w-14">DATE</span>
                  <span className="w-1.5" />
                  <span className="text-[9px] tracking-[0.12em] text-slate-600 flex-1">DESCRIPTION</span>
                  <span className="text-[9px] tracking-[0.12em] text-slate-600 w-24 hidden sm:block">CATEGORY</span>
                  <span className="text-[9px] tracking-[0.12em] text-slate-600 w-28 text-right">AMOUNT</span>
                  <span className="w-6" />
                </div>
                ...
                {account.transactions.map((tx) => (
                  <TransactionRow
                    key={tx.id}
                    transaction={tx}
                    onDelete={(id) => deleteTransaction(selectedAccId, id)}
                  />
                ))}
              </>
            )}
        ...
        {activeTab === "charts" && <AnalyticsTab account={account} />}
        {activeTab === "ai"     && <AIAdvisorTab account={account} />}
      </main>
